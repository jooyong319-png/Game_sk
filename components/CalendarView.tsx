...
export function CalendarView({ cursor, onCursorChange, games, wishlist }: Props) {
    ...
    <section className={styles.view}>
      ...
      <div className={styles.grid}>
        ...
        {cells.map((cell, i) => (
          <div
            key={i}
            className={`${styles.cell} ${!cell.inMonth ? styles.cellOther : ''} ${cell.isToday ? styles.cellToday : ''}`}
          >
            ...
            <div className={styles.cellDots}>
              ...
              {cell.games.length > 4 && (
                <span className={styles.cellMore}>+{cell.games.length - 4}</span>
              )}
            </div>
            {cell.games[0] && cell.games.length === 1 && (
              <Link href={`/game/${cell.games[0].id}`} scroll={false} className={styles.cellName}>
