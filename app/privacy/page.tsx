  ...
  return (
    <PageShell>
      <article className="legal">
        ...
        <ul>
          <li><strong>커뮤니티 게시글·댓글 작성 시</strong>: 제목·내용·첨부 이미지, 닉네임(선택), 어뷰징 방지를 위한 접속 IP. IP는 표시용으로 앞 2블록만 마스킹해 노출하고, 나머지는 <strong>해시 처리(원본 미저장)</strong>하여 도배 방지에만 사용합니다.</li>
        ...
        <h2>3. 이용 목적</h2>
        <ul>
          <li>커뮤니티·댓글·조회수 등 서비스 기능 제공 및 운영</li>
          <li>도배·어뷰징 방지(IP 해시 기반 게시 제한)</li>
        ...
        <ul>
          <li>Supabase — 게시글·댓글·이미지·조회수 데이터 저장</li>
        ...
        <h2>5. 보유 및 파기</h2>
        <p>게시글·댓글·첨부 이미지는 작성자의 삭제 또는 운영 정책에 따라 삭제될 때까지 보관됩니다.</p>

        <h2>6. 이용자의 권리 및 데이터 삭제 요청</h2>
        <p>게임 출시 캘린더(gcalen.com) 이용자는 다음과 같이 데이터 삭제를 요청할 수 있습니다.</p>
        <ul>
          <li><strong>직접 삭제</strong>: 커뮤니티 게시글·댓글은 작성 시 설정한 비밀번호를 입력해 이용자가 직접 삭제할 수 있습니다. 삭제 시 해당 게시글·댓글·첨부 이미지 및 연관 값이 즉시 제거됩니다.</li>
          <li><strong>이메일로 삭제 요청</strong>: 비밀번호를 잊었거나 본인이 올린 콘텐츠의 삭제를 원하시면 아래 문의처 이메일로 해당 게시글 링크와 함께 요청해 주세요. 본인 확인 후 지체 없이 삭제해 드립니다.</li>
          <li><strong>삭제되는 데이터</strong>: 게시글·댓글 내용, 첨부 이미지, 어뷰징 방지용 IP 해시값.</li>
