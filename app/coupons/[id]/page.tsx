...
import { getGameCoupons, getCouponPageGameIds, getCouponsLastUpdated } from '@/lib/coupons';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `${game.name_ko} 쿠폰 번호·기프트코드 최신 정리`;
  const desc = (active.length > 0
    ? `${game.name_ko} 최신 쿠폰 번호(기프트코드) ${active.length}개를 모았습니다. 코드를 복사해 게임 내 쿠폰 입력란에 넣으면 무료 보상을 받을 수 있어요. 만료·신규 쿠폰을 계속 업데이트합니다.`
    : `${game.name_ko} 쿠폰 번호(기프트코드) 정리. 현재 유효한 쿠폰은 없지만 최근 만료 코드와 사용법을 확인하고, 새 쿠폰이 나오면 이곳에서 가장 먼저 받아보세요.`
  ).slice(0, 158);

  return {
    title: { absolute: `${title} | Gcalen` },
    description: desc,
    keywords: [...couponKeywords(game.name_ko), '게임 쿠폰', '기프트코드', '쿠폰번호 모음'],
    alternates: { canonical: url },
    openGraph: {
      title, description: desc, url, type: 'article', siteName: '게임 출시 캘린더', locale: 'ko_KR',
      images: [{ url: game.image_url || '/og-image.png', alt: `${game.name_ko} 쿠폰` }],
    },
    ...
    '@graph': [
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: `${game.name_ko} 쿠폰`, item: url },
        ],
      },
      {
        '@type': 'FAQPage',
        mainEntity: [
          {
            '@type': 'Question',
            name: `${game.name_ko} 쿠폰(기프트코드)은 어떻게 사용하나요?`,
            acceptedAnswer: {
              '@type': 'Answer',
          ...
          {
            '@type': 'Question',
            name: `${game.name_ko} 쿠폰은 얼마나 자주 업데이트되나요?`,
            acceptedAnswer: {
              '@type': 'Answer',
    ...
    <PageShell>
      ...
      <section className={styles.wrap}>
        <nav className={styles.crumb} aria-label="위치">
          <Link href="/coupons">게임 쿠폰</Link> <span aria-hidden="true">›</span> {game.name_ko}
        </nav>

        <header className={styles.header}>
          <h1 className={styles.title}>
            <svg className="ic" aria-hidden="true"><use href="#ic-gift" /></svg>
            {game.name_ko} 쿠폰 번호 (기프트코드)
          </h1>
          <p className={styles.subtitle}>
            {active.length > 0
              ? `현재 사용 가능한 ${game.name_ko} 쿠폰(기프트코드) ${active.length}개입니다. 아래 코드를 복사해 게임 내 쿠폰 입력란 또는 공식 쿠폰 등록 페이지에 넣으면 무료 보상을 받을 수 있어요. 코드는 게임사 사정에 따라 만료되거나 조기 소진될 수 있습니다.`
              : `현재 사용 가능한 ${game.name_ko} 쿠폰은 없습니다. 아래는 최근 만료된 코드이며, 새 쿠폰이 나오면 이 페이지에 가장 먼저 업데이트됩니다.`}
          </p>
        </header>
        ...
        <div className={styles.usage}>
          ...
          <ol className={styles.steps}>
            ...
            <li>{game.name_ko}를 실행해 <strong>설정 › 쿠폰(기프트코드) 입력</strong> 메뉴 또는 공식 쿠폰 등록 페이지로 이동합니다.</li>
