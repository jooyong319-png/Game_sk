...
## 6-3. data/coupons.json (게임 쿠폰/기프트코드 — 리서처 관리) ⭐신규
게임별 **쿠폰 번호(기프트코드)**. 게임 상세 페이지의 '쿠폰' 섹션과 `/coupons` 허브에 자동 노출. "게임명 쿠폰" 검색 유입 + 콘텐츠 가치(AdSense)에 매우 유리하니 **적극 수집**.
```ts
{ schema_version: 1, last_updated: "YYYY-MM-DD",
  coupons: {
    "<게임 id>": [                         // games.json의 id와 동일해야 매칭됨
      { code: "쿠폰코드",
        reward: "보상 설명(예: 다이아 300, 성장물약 5개)",
        expires?: "YYYY-MM-DD",           // 만료일 있으면. 지나면 자동 숨김
        added?: "YYYY-MM-DD" }
    ]
  } }
```
- ⚠️ **실제로 유효한 코드만**: 게임사 공식 채널(공지·SNS·쿠폰 페이지)에서 확인된 것만. **코드 창작 절대 금지**(안 먹히는 코드는 신뢰·AdSense에 치명적).
- 주 대상: **모바일 MMORPG·가챠·방치형** 등 쿠폰을 자주 뿌리는 게임. 콘솔 AAA는 대부분 쿠폰 없음.
- **만료 관리**: 만료(`expires` 지난) 코드도 상세 페이지에 **'만료됨'으로 표시돼 노출**된다(최근 90일 이내). 그러니 **만료됐다고 바로 지우지 말 것**(콘텐츠·SEO 유지). **90일 넘게 지난** 오래된 코드만 주기적으로 정리.
- id가 games.json에 없으면 노출 안 됨 — 반드시 정확한 게임 id 사용.
