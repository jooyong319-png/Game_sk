...
## 개선 아이디어 (IDEAS)
- [디자이너 05-30] [밀도·정보중복·리스트] 리스트 뷰 날짜 그룹헤더(2026.05.30 (토))와 그 아래 카드 .release-date(📅 2026.05.30 (토))가 같은 날짜를 세로 ~80px 안에서 중복 표기(2건↑ 그룹은 헤더1+카드N=N+1회) → 그룹헤더가 날짜를 책임지는 맥락에선 카드 .release-date 날짜 숨김(D-day만 유지), 그룹헤더 없는 단독/검색 맥락에서만 카드 날짜 노출 분기. (기등록 '흡수행 날짜 단축'=패널/'모달 날짜 포맷'=모달과 별개 표면) script.js 렌더 분기 소규모, 신규 색 없음. 우선순위 낮음
