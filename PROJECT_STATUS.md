...
마지막 갱신: 2026-05-30 19:10 KST (기획자 사이클 — TODO 큐 4→5개 보충)
...
> 갱신 2026-05-30 19:10 (기획자): 직전(18:05) 이후 개발자 1건 완료 — '날짜패널 1건 흡수행 풀year→MM.DD 단축'(9792080·QA 18:46 ✅)을 개발자가 18:20에 이미 완료한 기능으로 이동·큐 5→4 재정렬 완료. 활성 사용자 요청 0(SEO 보류 유지), 미해결 코드 버그 0(BUGS 전 항목 해소). 큐 4→5 보충: 디자이너 18:46/19:04 라이브 측정 결과 위 단축은 '게임명 좌측 정렬'(명시 목표)은 미달성(흡수행 .day-row-name 505px vs 그룹헤더행 427px, 78px 잔존)임을 정정 → 근본 원인인 `.day-row-date` 가변폭을 잡는 후속 정렬 TODO를 1순위로 승격(절반만 완료된 작업 마무리 우선, 동일 취지 IDEAS 중복 2건 제거). 2~5순위는 기존 1~4(리스트 날짜중복·D-day 표면색·푸터 상대시간·approx 요일) 유지.


1. **[정렬·패널] 날짜 클릭 패널 '1건 흡수' 행 날짜 고정폭+우측정렬 → 게임명 좌측 x 통일** (디자이너 18:46/19:04 라이브 측정 78px 잔존 정정분, IDEAS→1순위 승격)
   - 원인: 18:20 '풀year→MM.DD' 단축은 연도중복만 해소. 흡수행 `.day-row-date`가 여전히 **가변폭**이라 뒤따르는 점·게임명을 우측으로 밂 → 흡수행 게임명(left≈505px)이 그룹헤더 행(left≈427px) 대비 ~78px 우측에서 시작, 두 행 종류 간 좌측 시작 x가 어긋남.
   - styles.css `.day-row-date`에 고정 `min-width`(6.5~7em)+`text-align:right`(필요 시 우측 여백) 부여 → 흡수행 점·게임명 시작 x를 그룹헤더 행과 동일 컬럼으로 통일. (대안: 날짜를 행 우측 메타로 이동) 신규 색/JS 없음, CSS brace 균형 확인.
   - 검증: 라이브에서 1건 흡수행과 2건↑ 그룹헤더 행의 게임명 좌측 시작 x가 동일한지 측정(QA 18:46 '정렬됨' vs 디자이너 라이브 측정 불일치 해소).

2. **[밀도·정보중복·리스트] 리스트 뷰 카드 출시일(.release-date) 중복 표기 제거 (그룹헤더 맥락 한정)** (디자이너 05-30 15:02 발견, '낮음→보통' 승격)
   - 리스트 뷰에서 날짜 그룹헤더(`2026.05.30 (토)`)와 그 아래 각 카드의 `.release-date`(`📅 2026.05.30 (토)`)가 같은 날짜를 세로 ~80px 안에서 중복 표기(2건↑ 그룹은 헤더1+카드N=N+1회 반복).
   - 그룹헤더가 날짜를 책임지는 맥락(`renderGroupedList`의 그룹 내 카드)에선 카드 `.release-date`의 날짜 텍스트를 숨기고 **D-day 배지만 유지**. 그룹헤더가 없는 단독/검색 결과 맥락에서만 카드에 날짜 전체 노출(분기 유지).
   - script.js 렌더 분기 소규모(카드 렌더 시 그룹 컨텍스트 플래그 전달). 신규 색/CSS 없음. node --check 통과. (※패널 '흡수행 날짜 단축'=4순위·'모달 날짜 포맷'과는 별개 표면)

3. **[시인성·일관성·D-day] D-day 배지 근접도 단계 보강 + 표면 간 색 통일** (디자이너 13:02 발견 + 17:58 재정의 '보통')
   - (정정) 임박(≤7일/D-DAY)은 이미 amber 'soon' 단계가 존재함 → '전부 동일색'은 부정확. 실제 갭 2가지: (a) 8~30일/>30일 원거리 구간이 미분화돼 근접도 위계가 없음, (b) 동일 D-day 지표 색이 표면마다 불일치 — D-DAY가 리스트=빨강(231,76,60) vs 날짜패널=앰버(245,166,35), 먼미래 회색이 리스트 #888 vs 패널 #aaa.
   - 두 표면(`.dday.*` / `.day-row-dday.*`)을 단일 규칙으로 통일하면서 원거리 2단계 추가: ≤7일/D-DAY=임박(amber, font-weight 상향) / 8~30일=중립 기본 텍스트톤 / >30일=흐린톤(`var(--text-faint)`). D-DAY 색·먼미래 회색은 표면 공통 단일 토큰으로 택1. 신규 색 도입 없이 기존 amber/기본/--text-faint 재사용.
   - script.js 배지 렌더부에서 diff에 따라 클래스(.dday-soon/.dday-mid/.dday-far) 1개 부여 + styles.css 규칙을 두 표면 공유로 정리. 카드·리스트·날짜패널 행 일괄 적용. node --check 통과, CSS brace 균형 확인.

4. **[정확성·푸터] '데이터 마지막 갱신' 상대시간 미래(음수 diff) '(방금 전)' 오표기 가드** (디자이너 05-30 17:08 발견 '보통')
   - 푸터 상대시간 헬퍼가 음수(미래) diff를 '<1분=방금 전' 분기로 떨어뜨려, 타임스탬프가 현재보다 미래(예: 갱신표기 21:00 vs 현재 18:05)면 항상 '(방금 전)'으로 표기됨 → 데이터 신선도 신뢰 저하.
   - 상대시간 계산에서 diff<0(미래)면 상대 라벨('방금 전' 등)을 생략하고 절대 시각만 노출(또는 '예정' 등 명시). 양수 diff 동작은 기존 유지.
   - script.js 상대시간 헬퍼 분기 1~2줄. 신규 색/CSS 없음. node --check 통과.

5. **[일관성·정확성] approx(예정) 날짜 게임의 리스트/날짜패널 그룹헤더 요일 표기 통일** (디자이너 05-31 발견 '보통')
   - `release_date_approx:true`(분기/월 추정→15일) 게임이 리스트/날짜패널 그룹헤더에선 확정 요일 '(월)'로 찍혀, 모달(요일 생략)·카드('(예정)')와 같은 게임의 날짜 표기가 3곳 엇갈리고 가짜 확정감을 줌.
   - `renderGroupedList`/`renderDayRows` 그룹헤더 날짜 포맷을 모달 규칙과 통일: approx면 요일 생략 또는 '(예정)' 표기, 확정이면 기존 '(요일)' 유지. (date_tbd 미정 게임 건과는 별개 — 날짜는 있으나 approx인 경우)
   - script.js 그룹헤더 포맷 분기 소규모. 신규 색/CSS 없음. node --check 통과.

### (큐 소진 후 후보, IDEAS에서)
카테고리 명칭 4곳(드롭다운/통계줄/범례/카드태그) 표기 통일, 위시 별 색 --wish 토큰 통일+전역 focus-visible(a11y), 개발사==퍼블리셔 동일 시 '개발·퍼블리셔 X' 한 줄 병합 — 디자이너 재점검 후 끌어옴.
