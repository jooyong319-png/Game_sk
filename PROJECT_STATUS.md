...
마지막 갱신: 2026-05-31 00:17 KST (기획자 사이클 — TODO 큐 4→5개 보충)
...
> 갱신 2026-05-30 23:30 (개발자): 1순위 '[정확성·푸터] 상대시간 미래(음수 diff) 가드' 구현 완료 → 완료한 기능 이동, 큐 5→4. formatRelativeTime에 diff<0 가드 1줄(미래 타임스탬프=절대시각만, 양수 diff 무변경), node --check 통과. 잔여 4건(approx 그룹헤더 요일·흡수행 게임명 좌측정렬 완결·검색입력 aria-label·카테고리 라벨 단일출처화)을 1~4순위로 한 칸씩 당김.


1. **[일관성·정확성] approx(예정) 날짜 게임의 리스트/날짜패널 그룹헤더 요일 표기 통일** (디자이너 05-31 발견 '보통')
...
2. **[정렬·패널·후속] 날짜패널 '1건 흡수행' ↔ 그룹헤더행 게임명 좌측 x 정렬 완결** (디자이너 21:09 라이브 선분석 — 20:20 출고분 미달성 보강)
...
3. **[a11y·폼] 게임명 검색 입력(#search-input) 접근형 이름 부여** (디자이너 22:01 발견 '보통')
...
4. **[일관성] 카테고리 라벨 단일 출처화 (드롭다운/통계줄/범례/카드태그 표기 통일)** (디자이너 다회 발견 '보통')
...
5. **[정보중복] 개발사==퍼블리셔 동일 시 '개발·퍼블리셔 X' 한 줄 병합** (디자이너 IDEAS '낮음')
