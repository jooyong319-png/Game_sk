...
마지막 갱신: 2026-05-30 13:10 KST (기획자 사이클 — TODO 큐 4→5개 보충)
...
> 갱신 2026-05-30 13:10 (기획자): 직전 1순위 '상세 모달 메타 행 라벨 폭 정렬'이 개발자 12:28 완료·QA ✅12:45·디자이너 12:58 라이브 재검증(데스크톱/모바일 5행 정렬 일치, 잘림 0)→완료한 기능으로 이동되어 큐 5→4. 활성 사용자 요청 0(SEO 보류 유지), 미해결 코드 버그 0(BUGS 전 항목 해소). 큐 4건이라 IDEAS '리스트 카드 비인터랙티브 메타칩(플랫폼·장르) 중립톤 통일'(비클릭 장르태그가 accent블루라 클릭가능 오인)을 작고 명확한 CSS TODO로 5순위 승격(IDEAS→큐) → 4→5 복구. 기존 4건(필터 초기화·날짜패널 우측 D-day 정렬·요일헤더 평일 대비·날짜패널 dot 색+모양)은 1~4순위 유지. 디자이너 데이터오류(PoE2)는 리서처 도메인이라 코드 TODO 미등재.


1. **[UX·발견성] 활성 필터 '초기화' 컨트롤 추가** (디자이너 05-30 09:02 '보통')
   - 카테고리/플랫폼/기간/검색/주칩/위시 6종 필터 중 비기본값이 1개라도 활성일 때만 '필터 초기화' 링크/버튼을 컨트롤 행에 노출. 클릭 시 6종 전부 기본값으로 일괄 리셋(카테고리·플랫폼=all, 기간=기본, 검색 input 비움, 주칩 해제, 위시 필터 해제) 후 재렌더. 결과 0건 빈 상태의 탈출구 역할도 겸함.
   - script.js에 (a) 비기본 필터 1개↑ 여부 판정 → 버튼 show/hide 토글, (b) 일괄 리셋 함수(기존 필터 상태 변수/이벤트 핸들러 재사용) 추가. 외형은 기존 링크/칩 톤 재사용, 신규 색 도입 없음. 작은 단위(~40줄 내) 유지, node --check 통과 확인.

2. **[정렬·패널] 날짜 클릭 패널 행 우측 '플랫폼·D-day' 고정 컬럼 정렬** (디자이너 05-30 10:05 '보통')
...
3. **[a11y·대비] 월간 캘린더 요일 헤더 평일(월~금) 색 대비 상향** (디자이너 05-31 '보통')
...
4. **[a11y] 날짜 클릭 패널 게임 행 카테고리 점(.day-row-dot) 색+모양 이중 인코딩** (디자이너 05-31 발견 '보통')
...
5. **[일관성·어포던스] 리스트 카드 비인터랙티브 메타칩(플랫폼·장르) 중립톤 통일** (디자이너 발견 '보통')
