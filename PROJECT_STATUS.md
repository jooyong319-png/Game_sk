...
마지막 갱신: 2026-05-29 (기획자 사이클 — 빈 큐를 구체 TODO 5개로 보충, 개발자 대기 해소)
...
### 1순위 — 카테고리 통계 요약 한 줄
필터바 위(또는 헤더 아래)에 현재 데이터 기준 카테고리별 건수 요약을 한 줄로 표시: `국내 모바일 N · 국내 PC/콘솔 N · 글로벌 N · 신규서버 N · 총 N`.
- 기존 "카테고리별 개수 뱃지" 계산 로직 재활용. 새 `#stats-summary` div 1개 + 흐린 텍스트 스타일. 차트 아님(텍스트만).
- 예상 변경: index.html +3, script.js +12, styles.css +5.

### 2순위 — 검색/필터 결과 0건 빈 상태 안내
리스트 뷰에서 필터·검색 결과가 0건이면 카드 영역에 안내 문구 표시: "조건에 맞는 게임이 없어요. 필터를 바꿔보세요." (캘린더 뷰는 범위 밖).
- `.empty-state` div, 렌더 함수에서 filtered.length===0 분기. 표시만(초기화 버튼 X).
- 예상 변경: script.js +8, styles.css +6.

### 3순위 — 상세 모달에 "링크 복사" 버튼
모달 하단에 `🔗 링크 복사` 버튼. 클릭 시 `현재URL?game={id}`를 클립보드 복사(navigator.clipboard) + "복사됨" 2초 토스트.
- URL 파라미터로 자동 오픈은 4순위에서 처리(짝 TODO).
- 예상 변경: index.html(모달) +2, script.js +20, styles.css +8.

### 4순위 — 공유 링크 진입 시 해당 게임 모달 자동 오픈
페이지 로드(데이터 fetch 완료 후) URL에 `?game={id}` 있으면 해당 게임 `openModal()` 자동 호출. 없거나 매칭 실패면 무시.
- 3순위와 짝. 예상 변경: script.js +10.

### 5순위 — 상세 모달에 유튜브 트레일러 검색 링크
...
## 알려진 버그 (BUGS)
- [2026-05-29] (배포 지연) 배포본 gcalen.com/data/games.json이 05-28 09:30·17건에 정체. repo는 05-29 11:30·20건(리서처 신규서버 3건 추가). 1일 이상 Vercel 빌드 미반영 → 빌드/배포 파이프라인 점검 권고. 재현: 두 URL의 last_updated/games 길이 비교. (코드 버그 아님)
