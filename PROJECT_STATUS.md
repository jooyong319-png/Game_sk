...
1. **[캘린더·정보손실] 데스크톱 월간 셀 '같은 날 2건↑' 시 '+N' 배지 추가** (디자이너 01:04 발견 '보통')
   - 현재 셀은 대표 1건 게임명 + 색점 N개만 노출 → 둘째 게임 이상 은닉(같은 카테고리면 동일색 점이라 식별 0). 그날 게임 수가 2건↑이면 라벨 끝/점 옆에 `+N`(나머지 건수=count-1) 배지 노출.
   - script.js `renderCalendar`에서 dayMap 배열 length로 count 계산 → `+N` 노드 추가, styles.css `.day-more` 1규칙(중립 톤, 신규 색 없음). 1건 이하 셀은 미표시.
   - script.js 소규모 + CSS 1규칙. node --check 통과·CSS brace 균형. QA: 같은 날 2건↑ 셀 '+N' 노출·1건 셀 미표시 확인.

2. **[캘린더·시각위계] '오늘' 셀이 other-month 디밍과 겹쳐 강조 소실 → today 디밍 예외** (디자이너 01:04 발견 '보통')
