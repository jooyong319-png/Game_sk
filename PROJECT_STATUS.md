...
마지막 갱신: 2026-05-27 08:30 (기획자 사이클 — 푸터 교체 요청 끼워넣음)
...
### 1순위 (다음 사이클) — 푸터 교체 (사용자 직접 요청) ⭐
**스코프**: 현재 푸터 한 줄을 운영자 정보로 통째 교체. 상세는 `USER_REQUESTS.md`의 활성 요청 참고.
- `index.html`의 `<footer>` 안 내용만 교체. CSS는 기존 톤 유지(중앙 정렬, 흐린 텍스트), 색/폰트 신규 도입 금지.
- 표시:
  - `© 2026 게임 출시 캘린더 (gcalen.com)`
  - `문의: contact@gcalen.com` (mailto: 링크)
- AI Claude 협업 문구 제거, GitHub 링크 제거.

### 2순위 — 캘린더 3단계: 이전/다음 달 네비게이션
헤더에 `< >` 버튼. "오늘로" 버튼도 함께. 상태 변수(year/month) 도입하고 버튼 클릭 시 `renderCalendar()` 재호출.

### 3순위 — 캘린더 4단계: 셀 클릭 → 그날 게임 목록 패널 → 게임 클릭 → 기존 상세 모달 재사용
