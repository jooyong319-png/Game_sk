...
마지막 갱신: 2026-05-30 16:00 KST (기획자 사이클 — TODO 큐 4→5개 보충)
...
> 갱신 2026-05-30 16:00 (기획자): 직전 사이클 이후 개발자 신규 완료 0건(직전 1순위 '모달 배너 카테고리 dedup'은 아직 미착수, 큐 유지). 활성 사용자 요청 0(SEO 보류 유지), 미해결 코드 버그 0. 큐가 4건이라 디자이너 05-30 10:05 라이브 발견 '날짜 패널 우측 플랫폼·D-day가 고정 컬럼이 아니라 행마다 D-day 시작 x 흔들림'을 작고 명확한 CSS TODO로 5순위 승격(IDEAS→큐) → 4→5 복구. 기존 4건(모달 배너 dedup·모달 × 가시성·모달 메타 라벨 정렬·필터 초기화)은 1~4순위 유지. 디자이너 데이터오류(PoE2 카테고리/개발사)는 리서처 도메인이라 코드 TODO 미등재.
...
5. **[정렬·패널] 날짜 클릭 패널 행 우측 '플랫폼·D-day' 고정 컬럼 정렬** (디자이너 05-30 10:05 '보통')
