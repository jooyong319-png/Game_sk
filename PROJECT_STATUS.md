...
마지막 갱신: 2026-05-30 14:11 KST (기획자 사이클 — TODO 큐 4→5개 보충)
...
## 완료한 기능
- [x] **[UX·발견성] 활성 필터 '초기화' 컨트롤 추가** — 비기본 필터(카테고리/플랫폼/기간/검색/주칩/위시 6종) 1개↑ 활성 시 `.controls-row`에 `↺ 필터 초기화` 버튼 노출, 클릭 시 6종 전부 기본값 일괄 리셋 후 재렌더(결과 0건 빈 상태 탈출구 겸). JS 주입 버튼+isAnyFilterActive/updateResetVisibility/resetFilters, .filter-reset 칩 톤(신규 색 없음) — 개발자 완료 2026-05-30 14:55
...
## 최근 변경 로그
- 2026-05-30 14:55 [개발자] 1순위 완료: **[UX·발견성] 활성 필터 '초기화' 컨트롤**. script.js 끝에 JS 주입 방식으로 `↺ 필터 초기화` 버튼을 `.controls-row`에 append(마크업 최소화) + `isAnyFilterActive()`(카테고리≠all/플랫폼≠all/기간≠365/검색input非공백/weekFilter≠null/wishlistOnly) + `updateResetVisibility()`(필터 컨트롤 change·input·click·searchClear에 추가 리스너로 호출, 버튼 hidden 토글) + `resetFilters()`(6종 기본값 리셋→applyWeekChips/applyWishlistChip/renderGames/renderCalendar 재사용). styles.css `.filter-reset`(칩 톤 #3a3e48/--text-dim/--accent, 신규 색 없음). node --check 통과, CSS brace 균형. TODO 큐에서 제거·이하 한 칸씩 당김.
