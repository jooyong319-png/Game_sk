...
마지막 갱신: 2026-05-30 16:00 (기획자 — TODO 큐 갱신, 디자이너 12:50 버그 1건 등재)
...
> 갱신 2026-05-30 16:00 (기획자): 직전 큐 3건 유지(어포던스·dot 이중인코딩·1건날짜 흡수). 활성 사용자 요청 0 확인. 디자이너 12:50 신규 발견 중 **'지난 날짜 셀 클릭 시 패널 헤더/내용 불일치 + 당일 게임 누락'을 correctness 버그로 1순위 등재**, '이벤트 셀 배경 강조'를 IDEAS→5순위 승격. 큐 3→5개.

1. **[버그] 지난 날짜 셀 클릭 시 패널 헤더 ↔ 목록 불일치 + 클릭한 날 게임 누락** (디자이너 12:50 발견)
   - 증상: 캘린더에서 지난 날짜(예: 5/27=007 퍼스트 라이트) 셀을 클릭하면 패널 헤더는 `2026.05.27 (수) 이후 출시 N건`이라 뜨는데, 실제 목록은 '오늘 이후'만 노출되어 (a)헤더와 내용 기준이 불일치, (b)클릭한 그 날 출시 게임(007)이 목록에서 누락됨.
   - 수정 방향(택1, a 권장): **(a)** renderDayPanel의 필터 기준을 `release_date >= 클릭일`로 통일(오늘이 아니라 클릭한 날짜를 바닥으로) → 클릭한 날 게임 포함되고 헤더 문구와 일치. **(b)** 지난 날짜 셀은 클릭 비활성/흐림 처리하고 헤더를 '오늘 이후'로 정정.
   - 현재 카테고리/플랫폼/기간/검색/위시 필터 반영은 유지. script.js renderDayPanel(또는 getActiveFilteredGames floor) 소규모 수정. 신규 색·문구 톤 변경 없음.

2. **[접근성] 날짜 셀 클릭 어포던스 + 키보드 접근** (디자이너 '보통')
...
3. **[접근성] 카테고리 점 색+모양 이중 인코딩** (디자이너 05-30 '보통')
...
4. **[밀도] 날짜 클릭 패널 — 1건 날짜는 날짜를 행 안으로 흡수** (디자이너 05-30 '보통')
...
5. **[스캔성] 출시 있는 셀에 옅은 면 강조** (디자이너 12:50/05-30 발견)
...
### (큐 소진 후 후보, IDEAS에서)
카테고리 명칭 4곳(드롭다운/통계줄/범례/카드태그) 표기 통일, D-day 배지 근접도 색 단계화(≤7 amber/≤30 중립/>30 흐린톤), 리스트 뷰 풀폭 행 카드 폴리시 — 디자이너 재점검 후 끌어옴.
