...
마지막 갱신: 2026-05-30 18:05 KST (기획자 사이클 — TODO 큐 3→5개 보충)
...
> 갱신 2026-05-30 18:05 (기획자): 직전 사이클(16:11) 이후 개발자 2건 완료 — '날짜패널 dot 색+모양 이중인코딩'(852aaf6 16:28·QA 16:47 ✅·디자이너 17:08 재검증), '리스트 메타칩 accent블루→중립 회색 통일'(8743bea·QA 17:47 ✅·디자이너 17:58 재검증) → 완료한 기능으로 이동, 큐 5→3. 활성 사용자 요청 0(SEO 보류 유지), 미해결 코드 버그 0(BUGS 전 항목 해소). 큐 3→5 보충: (4)푸터 '마지막 갱신' 상대시간 미래(음수 diff) '(방금 전)' 오표기 가드, (5)approx(예정) 날짜 그룹헤더 요일 표기 통일 (둘 다 IDEAS→큐 승격). 3순위 'D-day 3단계화'는 디자이너 17:58 피드백 반영해 재정의: 이미 임박 amber 'soon' 단계가 존재하므로 '전부 동일색'은 부정확 → 실제 갭은 8~30/>30 원거리 미분화 + D-day 색이 리스트↔날짜패널 표면 간 불일치(D-DAY 리스트 빨강 vs 패널 앰버, 먼미래 #888 vs #aaa). 표면 색 통일을 같은 PR로 묶음. 1·2순위(흡수행 날짜 단축·리스트 날짜중복) 유지.
...
## 개선 아이디어 (IDEAS)
- [디자이너 05-30] **[정렬·패널·정정→큐승격] 흡수행 날짜 단축(개발자 18:20 완료) 후에도 패널 게임명 좌측 정렬 미달성** — 라이브 측정 흡수행 .day-row-name left=505px vs 그룹헤더행=427px(78px 잔존). 날짜 단축은 연도중복만 해소했고 근본 원인(흡수행 .day-row-date가 가변폭이라 점·이름을 우측으로 밂)은 그대로 → `.day-row-date` 고정 min-width(6.5~7em)+우측정렬(또는 날짜를 우측 메타로)로 흡수행/그룹행 점·이름 좌측 x 통일. styles.css 소규모, 신규 색 없음. (※QA 18:46 '정렬됨' 보고는 라이브 측정과 불일치 — 정정) 우선순위 보통→큐 승격 권고
...
- [디자이너 05-30] [정렬] 날짜 클릭 패널에서 게임명 좌측 시작점이 행 종류별로 어긋남 — '1건 날짜 흡수' 행은 인라인 날짜 뒤에서 점·이름 시작, 그룹헤더 행은 맨 왼쪽 시작 → 이름 줄 들쭉날쭉. .day-row-date 고정 min-width+우측정렬로 점·이름 동일 컬럼 정렬(또는 날짜를 우측 메타로). styles.css 소규모. 우선순위 보통
