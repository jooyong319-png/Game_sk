...
마지막 갱신: 2026-05-30 20:00 KST (기획자 사이클 — TODO 큐 4→5개 보충)
...
> 갱신 2026-05-30 20:00 (기획자): 직전 사이클(16:00) 이후 개발자 1순위 '상세 모달 배너 카테고리 dedup' 완료(16:20)·QA ✅(10:46/날짜 표기 혼선 있으나 검증 통과)→완료한 기능으로 이동되어 큐 5→4. 활성 사용자 요청 0(SEO 보류 유지), 미해결 코드 버그 0(BUGS 전 항목 해소). 큐 4건이라 디자이너 [a11y·대비] '요일 헤더 평일(월~금) 색 #888 다크 대비 AA 미달'을 작고 명확한 CSS-only TODO로 5순위 승격(IDEAS→큐) → 4→5 복구. 기존 4건(모달 × 가시성·모달 메타 라벨 정렬·필터 초기화·날짜패널 우측 D-day 정렬)은 1~4순위 유지. 디자이너 데이터오류(PoE2 카테고리/개발사)는 리서처 도메인이라 코드 TODO 미등재.


1. **[a11y·모달] 상세 모달 닫기(×) 버튼 가시성 — 배너색 무관 고정 대비** (디자이너 12:50 '보통')
   - 상세 모달 닫기(×)가 밝은 카테고리 배너(녹색/보라)·이미지 위에서 `#aaa` 저대비로 거의 안 보임 → 닫기 발견성·터치 타겟 문제.
   - × 버튼에 반투명 다크 원형 배경(`rgba(0,0,0,0.45)`) + 흰색 아이콘 + 최소 44×44px 히트영역 부여 → 배너/이미지 색과 무관하게 고정 대비 확보(WCAG 2.5.5 터치타겟 겸 충족).
   - styles.css CSS-only 소규모(`.modal-close` 규칙 1곳). 반투명 흑/백만 사용, 신규 컬러 토큰 도입 없음. CSS brace 균형 확인.

2. **[정렬·모달] 상세 모달 메타 행 라벨 폭 정렬 (값 좌측 컬럼 통일)** (디자이너 05-31 16:50 '보통')
...
3. **[UX·발견성] 활성 필터 '초기화' 컨트롤 추가** (디자이너 05-30 09:02 '보통')
...
4. **[정렬·패널] 날짜 클릭 패널 행 우측 '플랫폼·D-day' 고정 컬럼 정렬** (디자이너 05-30 10:05 '보통')
...
5. **[a11y·대비] 월간 캘린더 요일 헤더 평일(월~금) 색 대비 상향** (디자이너 05-31 '보통')
