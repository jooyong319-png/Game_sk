...
마지막 갱신: 2026-05-29 (기획자 — 큐 3→5개 보충, 숫자불일치 버그 1순위)
...
> 갱신 2026-05-29 (기획자): 큐 3→5개 보충. 디자이너 18:05 보고의 숫자 불일치(통계줄 '총 29' vs 드롭다운 '전체 (25)')를 버그로 1순위 등재. 기존 운영자 요청 3건은 그대로 2~4순위 유지, IDEAS '선택 셀 위계 분리'를 5순위로 끌어옴. 모두 작고 명확(CSS+소규모 JS).

1. **[버그] 통계줄 총개수 vs 필터 드롭다운 '전체(N)' 숫자 불일치 수정**
   - 디자이너 실측: `#stats-summary`는 '총 29', 기간 드롭다운 '전체 (25)'로 숫자가 다름.
   - 원인 추정: stats는 전체 `allGames`(데이터 보관 180일 포함) 기준, 드롭다운 카운트는 다른 데이터셋(기간/필터 반영) 기준 → 같은 모집단을 쓰도록 정렬.
   - 어느 쪽이 '정답'인지 먼저 확정(권장: 둘 다 현재 데이터 전체 건수=동일 모집단). renderStatsSummary와 옵션 카운트 계산이 같은 배열을 참조하도록 1곳 수정. JS 소규모, 외형/문구 무변경.

2. **[캘린더] 날짜 클릭 패널을 한 줄 컴팩트 행으로 전환** (운영자 요청)
   - 날짜 패널의 카드형 목록을 밀도 높은 한 줄 행으로: `[카테고리 색점] 게임명 · 플랫폼 · D-day · ☆(위시)`.
   - 행 클릭 시 기존 `openModal()` 재사용, ☆는 위시 토글 재사용. 행 높이 40~48px(터치타겟). 표시 범위는 현행 유지('그날 이후 전체').
   - **리스트 뷰 탭 카드는 건드리지 말 것**(날짜 패널 렌더만 교체). CSS 위주 + 패널 템플릿 소규모 수정. (디자이너 '높음': 내부 스크롤 제거→인라인 확장도 이 작업에 묶어 처리)
