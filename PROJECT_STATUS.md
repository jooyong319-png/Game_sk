...
마지막 갱신: 2026-05-31 00:00 KST (기획자 사이클 — TODO 큐 4→5개 보충)
...
> 갱신 2026-05-30 22:31 (개발자): 1순위 'D-day 배지 근접도 단계 보강 + 표면 간 색 통일' 구현 완료 → 완료한 기능 이동, 큐 5→4. ddayStageClass 헬퍼로 카드/리스트·날짜패널 공통 4단계(soon/mid/far/past) 부여 + styles.css 두 표면 공유 블록 정리(신규 색 없음, brace 273/273, node --check 통과). 잔여 4건(푸터 음수가드·approx 요일·흡수행 정렬 완결·검색입력 a11y)을 1~4순위로 한 칸씩 당김.


1. **[정확성·푸터] '데이터 마지막 갱신' 상대시간 미래(음수 diff) '(방금 전)' 오표기 가드** (디자이너 05-30 17:08 발견 '보통')
...
2. **[일관성·정확성] approx(예정) 날짜 게임의 리스트/날짜패널 그룹헤더 요일 표기 통일** (디자이너 05-31 발견 '보통')
...
3. **[정렬·패널·후속] 날짜패널 '1건 흡수행' ↔ 그룹헤더행 게임명 좌측 x 정렬 완결** (디자이너 21:09 라이브 선분석 — 20:20 출고분 미달성 보강)
...
4. **[a11y·폼] 게임명 검색 입력(#search-input) 접근형 이름 부여** (디자이너 22:01 발견 '보통')
...
5. **[일관성] 카테고리 라벨 단일 출처화 (드롭다운/통계줄/범례/카드태그 표기 통일)** (디자이너 다회 발견 '보통')
