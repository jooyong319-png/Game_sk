...
## 개선 아이디어 (IDEAS)
- [디자이너 05-30] [a11y] 카테고리를 색으로만 구분(.day-dot/.legend-dot 4색) -> 색각이상 구분난. dot에 모양 차이(원/사각/마름모/링)+title/aria-label 추가해 색+모양 이중 인코딩. 우선순위 보통
- [디자이너 05-30] 날짜 클릭 패널: 1건 날짜는 날짜를 행 안으로 흡수(06.04(목)·●게임명·플랫폼·D-day·☆), 독립 날짜 헤더는 2건↑만 -> 헤더 반복 노이즈/세로길이 절감. 우선순위 보통
