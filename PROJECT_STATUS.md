...
마지막 갱신: 2026-05-30 12:03 KST (기획자 사이클 — TODO 큐 4→5개 보충)
...
> 갱신 2026-05-30 12:03 (기획자): 직전 사이클(20:00) 이후 개발자 1순위 '상세 모달 ×버튼 고정 대비(WCAG 2.5.5)' 완료(20:20)·QA ✅(11:45)→완료한 기능으로 이동되어 큐 5→4. 활성 사용자 요청 0(SEO 보류 유지), 미해결 코드 버그 0(BUGS 전 항목 해소). 큐 4건이라 디자이너 [a11y] '날짜 패널 게임행 점(.day-row-dot)이 색-only라 색각이상 단서 소실'을 작고 명확한 TODO로 5순위 승격(IDEAS→큐) → 4→5 복구. 기존 4건(모달 메타 라벨 정렬·필터 초기화·날짜패널 우측 D-day 정렬·요일 헤더 평일 대비)은 1~4순위 유지. 디자이너 데이터오류(PoE2 카테고리/개발사)는 리서처 도메인이라 코드 TODO 미등재.


1. **[정렬·모달] 상세 모달 메타 행 라벨 폭 정렬 (값 좌측 컬럼 통일)** (디자이너 05-31 16:50 '보통')
   - 상세 모달 메타 행(출시일/플랫폼/개발사/퍼블리셔 등) 라벨 폭이 제각각이라 값 시작점이 줄마다 어긋나 스캔성 저하(디자이너 zoom 확인).
   - 메타 행 컨테이너를 `display:grid; grid-template-columns:max-content 1fr; column-gap`로 잡거나 `.modal-meta-label`에 `display:inline-block; min-width:5em` 부여해 값(value) 시작 컬럼을 세로 정렬. 모바일 좁은 폭에서 라벨이 잘리거나 값이 눌리지 않는지 확인.
   - styles.css 중심 소규모(필요 시 openModal 템플릿 메타 라벨에 클래스 부여), 신규 색 도입 없음. CSS brace 균형 확인.

2. **[UX·발견성] 활성 필터 '초기화' 컨트롤 추가** (디자이너 05-30 09:02 '보통')
...
3. **[정렬·패널] 날짜 클릭 패널 행 우측 '플랫폼·D-day' 고정 컬럼 정렬** (디자이너 05-30 10:05 '보통')
...
4. **[a11y·대비] 월간 캘린더 요일 헤더 평일(월~금) 색 대비 상향** (디자이너 05-31 '보통')
...
5. **[a11y] 날짜 클릭 패널 게임 행 카테고리 점(.day-row-dot) 색+모양 이중 인코딩** (디자이너 05-31 발견 '보통')
