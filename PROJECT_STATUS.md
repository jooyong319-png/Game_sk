...
마지막 갱신: 2026-05-30 12:00 KST (기획자 사이클 — TODO 큐 4→5개 보충)
...
> 갱신 2026-05-30 12:00 (기획자): 직전 1순위 '[스캔성] 출시 있는 셀 옅은 면+좌측 악센트 강조(day-has)'는 개발자 08:29 완료·QA 08:44 ✅로 완료한 기능 이동 확인 → 큐 5→4로 줄어듦. 디자이너 09:02 라이브 발견(임팩트 1) '활성 필터 6종을 한 번에 되돌리는 필터 초기화 컨트롤 부재(0건 빈 상태 탈출구 겸)'를 5순위로 승격해 4→5 복구. 활성 사용자 요청 0(SEO 보류 유지), 미해결 코드 버그 0. 잔여 4건(’오늘’ 라벨 색통일·모달 배너 카테고리 dedup·모달 × 가시성·모달 메타 라벨 정렬)은 1~4순위 유지. 디자이너 데이터오류 2건(PoE2 카테고리/개발사 필드)은 리서처 도메인이라 코드 TODO 미등재.


1. **[일관성·모달] 상세 모달 상단 배너 카테고리 텍스트 중복 제거** (디자이너 05-31 '보통')
   - 상세 모달 상단 컬러 배너(placeholder) 안 카테고리 문구 + 바로 아래 본문 카테고리 pill이 동일 문구 2회 노출. 리스트 카드는 16:17 커밋에서 배너 텍스트 제거(컬러 4px바)로 dedup 완료됐으나 모달만 미적용.
   - 모달 배너 placeholder의 카테고리 텍스트 span 제거(컬러만 유지) → 카테고리는 본문 pill 1회만. 리스트 카드 dedup 패턴과 일관. script.js openModal 템플릿 소규모 + styles.css 정리, 신규 색 없음.

2. **[a11y·모달] 상세 모달 닫기(×) 버튼 가시성 — 배너색 무관 고정 대비** (디자이너 12:50 '보통')
...
3. **[정렬·모달] 상세 모달 메타 행 라벨 폭 정렬 (값 좌측 컬럼 통일)** (디자이너 05-31 16:50 '보통')
...
4. **[UX·발견성] 활성 필터 '초기화' 컨트롤 추가** (디자이너 05-30 09:02 '보통')
...
## 개선 아이디어 (IDEAS)
- [디자이너 05-30] [정렬·패널] 날짜 클릭 패널 행 우측 '플랫폼·D-day'가 고정 컬럼이 아니라 플랫폼 폭(PC/Android/Switch 2)에 따라 D-day 시작 x가 행마다 흔들림 → D-day(1차 지표) 세로 스캔 비용↑. .day-row-plat에 min-width(~5em)+text-align:right 또는 plat+dday를 고정폭 우측 그룹으로 묶어 D-day 좌측 컬럼 정렬(styles.css 1~2줄). (패널 게임명 좌측 정렬·모달 메타 정렬과 별개 표면) 우선순위 보통
