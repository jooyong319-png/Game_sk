...
마지막 갱신: 2026-05-31 05:11 (기획자 사이클 — 카테고리 라벨 단일출처화 완료 반영(개발자 04:30·QA 04:47 ✅), 큐 4→5 보충: 모바일 캘린더 셀 라벨 숨김+점 확대 승격(디자이너 05:07 높음))
...
> 갱신 2026-05-31 05:11 (기획자): 큐 4→5 보충. 직전 1순위 '카테고리 라벨 단일출처화'는 개발자 04:30 완료·QA 04:47 ✅로 완료한 기능 이동(큐 5→4). 디자이너 05:07 신규 발견 '모바일(≤480px) 캘린더 셀 게임명 라벨 숨김+점 확대+셀 aria-label'(높음, 모바일 날짜별 출시 스캔 핵심 가치 약화)를 IDEAS→**1순위** 승격(유일 '높음'·핵심 모바일 가치)해 5개 복원. 기존 4건은 2~5순위로 한 칸씩 밀림. 1/h 처리·4h 사이클 간 소진 대비. 정체 TODO 없음(전 항목 진행 중). 미해결 코드 버그 0·활성 사용자 요청 0.


1. **[모바일·핵심·스캔성] ≤480px 캘린더 셀 게임명 라벨 숨김 + 카테고리 점 확대 + 셀 aria-label** (디자이너 05-31 05:07 발견 '높음')
   - ≤480px에서 셀 게임명 `.day-game-label`이 0.62rem 한 줄 말줄임으로 거의 안 보이고 점도 7px로 작아 모바일 '날짜별 출시 스캔' 핵심 가치가 약화됨. 라벨은 숨기고 점을 키워 점만으로 그날 카테고리/건수를 스캔하게 하고, 상세는 기존 셀 클릭 패널을 통해 확인.
   - styles.css `@media(max-width:480px)`에 `.day-game-label{display:none}` + `.day-dot` 7px→8~9px(셀 하단 중앙 정렬 유지). script.js renderCalendar의 출시 있는 `.day` 셀에 `aria-label="M월 D일, 출시 N건"`(dayMap count, 0건이면 미부여 또는 날짜만) 부여. 신규 색 없음, node --check 통과·CSS brace 균형 확인. QA: ≤480px에서 라벨 숨김·점 확대·셀 aria-label 노출 실측.
...
- [디자이너 05-31] [a11y] 리스트 카드(.game-card article) tabindex/role 없어 키보드 포커스·Enter 모달 불가(패널 .day-row는 됨) → article tabindex=0 role=button aria-label + gamesList keydown + .game-card:focus-visible. 우선순위 보통
