...
마지막 갱신: 2026-05-29 (기획자 — 사용자요청 2건 완료처리, TODO 큐 0→5개 보충)
...
> 갱신 2026-05-29 (기획자): 사용자 요청 2건(날짜클릭 '이후 목록' / 가독성·UX 4테마)이 모두 완료되어 큐가 비었음 → USER_REQUESTS 아카이브로 이동. 디자이너 '높음' 제안들을 작고 명확한 1시간 단위 TODO 5개로 끌어와 큐를 채움. 대부분 CSS+소규모 JS, 회귀 위험 낮음.

1. **[캘린더] 진입 시 '오늘 이후 가장 가까운 출시 달'로 초기화**
   - 첫 로드 시 현재 달(출시 거의 없음)이 아니라 게임이 있는 달부터 보이게.
   - `loadData` 직후, `games` 중 `release_date >= 오늘`인 가장 이른 게임의 연/월로 `calendarYear`/`calendarMonth` 설정 후 `renderCalendar()`. 해당 게임 없으면 현재 달 유지.
   - 사용자가 ‹ ›로 이동한 뒤에는 재초기화하지 말 것(최초 1회만). JS ~10줄, 외형 변경은 초기 표시 월만.

2. **[캘린더] 출시 0건인 달 빈 상태 안내**
   - 현재 보고 있는 달에 출시 게임 0건이면 그리드 아래(또는 위)에 한 줄 안내: `이 달 출시 일정이 없어요. ‹ ›로 다른 달을 살펴보세요.`
   - `renderCalendar`에서 그 달 게임 수 카운트해 0이면 `.calendar-empty-note` 표시/아니면 숨김. CSS는 기존 흐린 텍스트 톤 재사용(신규 색 X). JS 소규모 + CSS 1블록.

3. **[캘린더] 날짜 클릭 패널을 한 줄 컴팩트 행으로 전환** (운영자 요청)
   - 현재 날짜 패널의 카드형 목록을 밀도 높은 한 줄 행으로: `[카테고리 색점] 게임명 · 플랫폼 · D-day · ☆(위시)`.
   - 행 클릭 시 기존 `openModal()` 재사용, ☆는 위시 토글 재사용. 행 높이 40~48px(터치타겟). 표시 범위는 현행 유지('그날 이후 전체').
   - **리스트 뷰 탭 카드는 건드리지 말 것**(날짜 패널 렌더만 교체). CSS 위주 + 패널 템플릿 소규모 수정.

4. **[헤더] 좌측정렬 + 컴팩트화로 캘린더를 첫 화면 위로** (운영자 요청)
...
5. **[뷰] 진입 시 기본 뷰를 캘린더로 고정** (운영자 요청, 보통)
   - `localStorage 'gcalen.view'`가 비어있을 때(최초 방문) 기본값을 `'calendar'`로. 이미 선택 기억 로직 있으니 기본값만 조정.
   - 기존 토글/기억 동작은 유지. JS 1~2줄.

### (큐 소진 후 후보, IDEAS에서)
리스트 뷰 가로 공간 낭비 해소(날짜그룹 풀폭 행 카드), 카드 상단 배너 콤팩트화는 이미 일부 반영 → 나머지 폴리시는 디자이너 재점검 후 끌어옴.
