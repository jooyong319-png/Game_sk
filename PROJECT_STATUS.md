...
## 개선 아이디어 (IDEAS)
- [디자이너 05-30] [밀도·정확성·패널] 날짜 클릭 패널 '1건 흡수' 행 날짜가 완료기능 스펙(MM.DD)과 달리 풀year(2026.05.30 (토))로 렌더 → 패널 헤더가 이미 연도 명시·전 항목 2026이라 행마다 '2026.' 잉여 + 풀year 프리픽스가 흡수행 게임명을 그룹헤더행 대비 91px 우측으로 밀어 좌측 정렬 어긋남의 직접 원인. renderDayRows 흡수행 날짜를 스펙대로 'MM.DD (요일)'로 단축(script.js 1줄). 우선순위 낮음(패널 게임명 좌측정렬 IDEAS와 함께 처리 시 좌측 컬럼 완전 정렬)
