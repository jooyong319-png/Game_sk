...
마지막 갱신: 2026-05-30 21:20 (개발자 — 접근성 셀 클릭 어포던스+키보드 구현, 큐 5→4)
...
> 갱신 2026-05-30 21:00 (기획자): 직전 큐의 1순위(지난 날짜 셀 클릭 버그) 개발자 16:30 완료·QA 01:47 ✅ → 완료한 기능으로 이동 확인. 잔여 4건 유지. 활성 사용자 요청 0·미해결 코드 버그 0 확인. 4시간(4사이클) 윈도 동안 개발자 소진 대비 IDEAS의 디자이너 발견 1건(’오늘’ 라벨 amber↔today 파랑 색충돌)을 5순위로 승격. 큐 4→5개.
...
## 개선 아이디어 (IDEAS)
- [디자이너 05-31] **[버그·높음]** D-day가 KST에서 전 항목 +1 (오늘 출시가 D-DAY 대신 D-1). 원인: release_date를 `new Date('YYYY-MM-DD')`=UTC자정으로 파싱하나 today는 로컬자정 → +9h 차로 ceil 시 하루 밀림. 캘린더 '오늘(30)' 하이라이트와 'D-1' 모순, KST에서 D-DAY 라벨 미표시. → release_date를 `new Date(str+'T00:00:00')` 로컬 파싱으로 통일(diff 계산 라인 205·261·386·71-73·132). 도메인: 개발. 우선순위 높음
