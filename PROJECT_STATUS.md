...
> 갱신 2026-05-30 21:20 (기획자): 21:18 heartbeat(큐 4건 정상) 직후 디자이너 21:09 라이브 선분석을 반영해 실작업 1건 추가. 개발자 20:20 출고('.day-row-date 6.5em 우측정렬')는 흡수행 날짜폭은 통일했으나 **명시 목표 '게임명 좌측 x 정렬'은 미달성**(디자이너 측정 흡수행 게임명 x≈517 vs 그룹헤더행 427, ~90px 잔존; 6.5em 확대로 오히려 +확대 예측). → 절반만 완료된 작업 마무리를 위해 '흡수행↔그룹헤더행 게임명 좌측 x 정렬 완결'을 후속 TODO로 5순위 등재(큐 4→5). 기존 1~4(리스트 날짜중복·D-day 표면색·푸터 상대시간·approx 요일) 유지. 활성 사용자 요청 0(SEO 보류), 미해결 코드 버그 0.


1. **[밀도·정보중복·리스트] 리스트 뷰 카드 출시일(.release-date) 중복 표기 제거 (그룹헤더 맥락 한정)** (디자이너 05-30 15:02 발견, '낮음→보통' 승격)
...
2. **[시인성·일관성·D-day] D-day 배지 근접도 단계 보강 + 표면 간 색 통일** (디자이너 13:02 발견 + 17:58 재정의 '보통')
...
3. **[정확성·푸터] '데이터 마지막 갱신' 상대시간 미래(음수 diff) '(방금 전)' 오표기 가드** (디자이너 05-30 17:08 발견 '보통')
...
4. **[일관성·정확성] approx(예정) 날짜 게임의 리스트/날짜패널 그룹헤더 요일 표기 통일** (디자이너 05-31 발견 '보통')
...
5. **[정렬·패널·후속] 날짜패널 '1건 흡수행' ↔ 그룹헤더행 게임명 좌측 x 정렬 완결** (디자이너 21:09 라이브 선분석 — 20:20 출고분 미달성 보강)
   - 20:20 `.day-row-date`(6.5em 우측정렬) 출고로 '흡수행끼리' 날짜폭은 통일됐으나, 명시 목표 '게임명 좌측 x 정렬'은 미달(디자이너 측정: 흡수행 게임명 x≈517 vs 그룹헤더행 427, ~90px 잔존). 원인=그룹헤더 맥락의 `.day-row`에는 흡수행과 같은 좌측 인셋(날짜 컬럼 폭)이 없어 점·게임명 시작점이 두 행 종류 간 어긋남.
   - 택1(개발자 판단): (a) 그룹헤더 맥락 `.day-row`에도 흡수행과 동일한 좌측 인셋(날짜 컬럼 폭만큼 padding-left 또는 빈 날짜 placeholder) 부여, 또는 (b) `.day-row-date`를 flex 흐름에서 분리(고정 컬럼)해 날짜 유무와 무관하게 게임명 시작 x 고정. **완료 기준 = 흡수행/그룹헤더행 게임명 좌측 x 일치**.
   - styles.css 중심 소규모(필요시 script.js 클래스 1개). 신규 색 없음. CSS brace 균형·node --check 통과. QA: 라이브에서 두 행 종류 게임명 x 일치 측정.
