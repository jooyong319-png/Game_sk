...
마지막 갱신: 2026-05-30 16:11 KST (기획자 사이클 — TODO 큐 4→5개 보충)
...
> 갱신 2026-05-30 16:11 (기획자): 직전 1순위 '월간 캘린더 요일 헤더 평일(월~금) 색 대비 상향'이 개발자(726bc8c) 완료·QA 15:45 ✅·디자이너 15:59 라이브 재검증(월~금 computed rgb(170,170,170)=#aaa 반영 확인)→완료한 기능으로 이동되어 큐 5→4. 활성 사용자 요청 0(SEO 보류 유지), 미해결 코드 버그 0(BUGS 전 항목 해소). IDEAS 'D-day 배지 근접도 색 단계화'(디자이너 13:02 발견, 임박/원거리 동일색→시각강조 약함)를 작고 명확한 TODO로 5순위 승격(IDEAS→큐) → 4→5 복구. 기존 4건(날짜패널 dot 색+모양·메타칩 중립톤·흡수행 날짜 단축·리스트 날짜중복)은 1~4순위 유지.
...
3. **[시인성·D-day] D-day 배지 근접도 색 3단계화** (디자이너 13:02 발견 '낮음→보통')
   - 현재 D-day 배지가 임박(D-2)·원거리(D-217)·D-DAY가 모두 동일 색이라 '곧 나올 게임' 시각 강조가 약함. 캘린더/리스트/패널 핵심 지표인데 근접도 위계가 없음.
   - D-day 계산값(diff days) 기준 3단계로 색 분기: ≤7일(및 D-DAY=0)=임박 amber(기존 .day-soon amber 재사용)+font-weight 상향, 8~30일=중립 기본 텍스트톤, >30일=흐린톤(var(--text-faint)). 신규 색 도입 없이 기존 amber/기본/--text-faint 3색만 재사용.
   - script.js D-day 배지 렌더부에서 diff에 따라 클래스(.dday-soon/.dday-mid/.dday-far) 1개 부여 + styles.css 3개 규칙. 배지가 렌더되는 표면(카드·날짜패널 행 등) 일괄 적용. node --check 통과, CSS brace 균형 확인.
...
## 개선 아이디어 (IDEAS)
- [디자이너 05-30] [일관성·시인성·표면불일치] 동일 D-day 지표가 리스트↔날짜패널에서 다른 색 — D-DAY 리스트=빨강(231,76,60) vs 패널=앰버(245,166,35), 먼미래 회색 리스트=#888 vs 패널=#aaa(+리스트 먼미래#888이 패널 '출시됨' 회색과 동일톤). `.dday.*`/`.day-row-dday.*` 두 스타일 트리를 단일 토큰으로 통일(D-DAY 색·먼미래 회색 택1). 큐3 'D-day 3단계화'와 같은 PR 권장. 우선순위 보통
- [디자이너 05-30] [정확성·푸터] 푸터 '데이터 마지막 갱신' 상대시간이 미래 시각(예 21:00 vs 현재 17:01)에도 '(방금 전)'으로 표기 → 상대시간 헬퍼가 음수(미래) diff를 '<1분=방금 전' 분기로 떨어뜨려 미래로 스큐된 타임스탬프가 늘 '방금 전'이 됨(데이터 신선도 신뢰 저하). 음수 diff면 상대라벨 생략(절대시각만) 또는 데이터가 실제 갱신시각 사용. (헤더↔푸터 갱신표기 '중복/포맷 통일' 건과 별개=상대시간 계산 정확성) 우선순위 보통
...
- [디자이너 05-31] [일관성·정확성] approx(예정) 날짜 게임의 리스트/날짜패널 그룹 헤더가 확정 요일 '(월)' 표기 → 모달은 요일 생략·카드는 '(예정)'이라 같은 게임 날짜 표기가 3곳 엇갈림+가짜 확정감. renderGroupedList/renderDayRows 그룹헤더 요일을 모달 규칙(approx면 요일 생략 or '(예정)')과 통일. (date_tbd 미정게임 건과 별개=날짜 있으나 approx) 우선순위 보통
