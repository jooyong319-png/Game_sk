...
마지막 갱신: 2026-05-27 07:50 (개발자 사이클 — 캘린더 1단계 완료)
...
### 1순위 (다음 사이클) — 캘린더 2단계: 셀에 그날 출시 게임 점/뱃지 표시
**스코프 (이것만 구현, 추가 금지):**
- 1단계에서 만든 각 `.day` 셀 내부에 그날 release_date인 게임의 카테고리 색상 점(dot) 표시.
- 한 셀에 최대 3개까지만 점 표시. 4개 이상이면 마지막 점 자리에 "+N" 텍스트.
- 점 색상은 기존 `category-*` 색 팔레트 재사용.
- 호버 시 tooltip(`title` 속성)으로 게임명 노출 (간단히).
- 이전 달/다음 달 칸의 게임은 표시 안 함 (현재 월 셀만).

**하지 않을 것 (다른 단계):**
- 월 이동 (3단계), 셀 클릭 동작 (4단계), 뷰 토글 (5단계), 필터 연동 (통합 단계).
