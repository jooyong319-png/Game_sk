...
마지막 갱신: 2026-05-30 22:12 KST (기획자 사이클 — TODO 큐 4→5개 보충)
...
> 갱신 2026-05-30 22:12 (기획자): 직전 사이클(21:20) 이후 개발자 1순위 '리스트 뷰 카드 출시일 중복 표기 제거' 완료(21:29)·QA ✅(21:45)→완료한 기능 이동으로 큐 5→4, 잔여 4건이 1~4순위로 당겨짐. 활성 사용자 요청 0(SEO 보류), 미해결 코드 버그 0(BUGS 전 항목 ✅). 디자이너 22:01 라이브 재검증: 최근 출고 4건 모두 정상 ✅, 신규 a11y 3건 제기(검색입력 접근형 이름 '보통'·skip-link '낮음'·theme-color '낮음'). 가장 임팩트 큰 '검색 입력 접근형 이름(aria-label) 부여'를 작고 명확한 a11y TODO로 5순위 승격(IDEAS→큐) → 4→5 복구. 기존 4건(D-day 색통일·푸터 음수가드·approx 요일·흡수행 정렬) 1~4순위 유지.


1. **[시인성·일관성·D-day] D-day 배지 근접도 단계 보강 + 표면 간 색 통일** (디자이너 13:02 발견 + 17:58 재정의 '보통')
...
2. **[정확성·푸터] '데이터 마지막 갱신' 상대시간 미래(음수 diff) '(방금 전)' 오표기 가드** (디자이너 05-30 17:08 발견 '보통')
...
3. **[일관성·정확성] approx(예정) 날짜 게임의 리스트/날짜패널 그룹헤더 요일 표기 통일** (디자이너 05-31 발견 '보통')
...
4. **[정렬·패널·후속] 날짜패널 '1건 흡수행' ↔ 그룹헤더행 게임명 좌측 x 정렬 완결** (디자이너 21:09 라이브 선분석 — 20:20 출고분 미달성 보강)
...
5. **[a11y·폼] 게임명 검색 입력(#search-input) 접근형 이름 부여** (디자이너 22:01 발견 '보통')
   - 헤더 검색 input(`#search-input`, index.html L69)에 placeholder만 있고 label·aria-label이 없어 스크린리더가 용도를 안내 못함(WCAG 4.1.2/3.3.2). select 3종은 label 래핑으로 이름이 있으나 검색만 누락.
   - `#search-input`에 `aria-label="게임명 검색"` 부여(또는 시각적 숨김 `<label for="search-input">`). placeholder는 보조로 유지, 외형 무변경·신규 색 없음.
   - index.html 속성 1줄(정적 마크업이라 script.js 불필요). node --check 통과(JS 무변경이면 생략 가능), HTML 유효성 확인.
