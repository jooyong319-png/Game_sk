...
마지막 갱신: 2026-05-27 20:00 KST (기획자 사이클 — TODO 큐 보충 5개)
...
### 1순위 — 캘린더 4단계: 셀 클릭 → 그날 게임 목록 패널 → 게임 클릭 → 기존 상세 모달 재사용
- 캘린더 그리드 아래에 `#day-detail-panel` 영역 신설 (초기엔 빈 상태/숨김)
- 셀 클릭 시 해당 날짜의 게임들을 카드 형태로 패널에 렌더(이미지 없으면 카테고리 색 박스 + 이름 + 카테고리 뱃지)
- 같은 셀 다시 클릭 시 패널 닫힘 토글, 다른 셀 클릭 시 내용 교체
- 패널 내 게임 클릭 → 기존 `openModal()` 그대로 재사용 (새 모달 만들지 말 것)
- 점이 없는 빈 날짜 클릭 시 "이 날짜에 출시 예정 게임 없음" 안내
- 이전/다음 달 dim 칸은 클릭 비활성(또는 클릭 시 해당 월로 이동) — 한 가지로 골라 일관되게
- 모바일에서 패널은 캘린더 아래 풀폭으로 표시

### 2순위 — 캘린더 5단계: 캘린더/리스트 뷰 토글
