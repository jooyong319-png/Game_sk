...
마지막 갱신: 2026-05-30 15:05 KST (기획자 사이클 — TODO 큐 4→5개 보충)
...
> 갱신 2026-05-30 15:05 (기획자): 직전 1순위 '날짜 클릭 패널 행 우측 플랫폼·D-day 고정폭 정렬'이 개발자(fd6466f) 완료·QA 15:05 ✅(node --check·CSS brace 273/273·`.day-row-plat` min-width:4.5em+우측정렬 소스 확인)→완료한 기능으로 이동되어 큐 5→4. 활성 사용자 요청 0(SEO 보류 유지), 미해결 코드 버그 0(BUGS 전 항목 해소). 디자이너 15:02 신규 발견 '리스트 그룹헤더 날짜↔카드 .release-date 날짜 중복(2건↑ 그룹서 N+1회 반복)'을 작고 명확한 TODO로 5순위 승격(IDEAS→큐) → 4→5 복구. 기존 4건(요일헤더 평일 대비·날짜패널 dot 색+모양·메타칩 중립톤·흡수행 날짜 단축)은 1~4순위 유지. 디자이너 데이터오류(PoE2·Square Enix 등)는 리서처 도메인이라 코드 TODO 미등재.
...
### (큐 소진 후 후보, IDEAS에서)
카테고리 명칭 4곳(드롭다운/통계줄/범례/카드태그) 표기 통일, D-day 배지 근접도 색 단계화(≤7 amber/≤30 중립/>30 흐린톤), 위시 별 색 --wish 토큰 통일+전역 focus-visible(a11y), 개발사==퍼블리셔 동일 시 '개발·퍼블리셔 X' 한 줄 병합 — 디자이너 재점검 후 끌어옴.
