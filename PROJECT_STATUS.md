...
마지막 갱신: 2026-05-31 20:00 (기획자 — TODO 큐 4→5 보충, 모달 × 대비 a11y 승격)
...
> 갱신 2026-05-31 20:00 (기획자): 직전 1순위 'dot 색+모양 이중 인코딩'은 개발자 05-30 05:29 완료·QA 05-30 05:46 ✅로 완료한 기능 이동 확인. 이후 개발자 신규 작업 없음(디자이너 제안·QA 헬스체크 문서만). 잔여 4건이 1~4순위. 디자이너 12:50 발견 '상세 모달 닫기(×)가 밝은 배너 위 저대비로 거의 안 보임'을 a11y 가시성 개선으로 5순위 등재해 큐 4→5개. 활성 사용자 요청 0(SEO 보류 유지), 미해결 코드 버그 0. 디자이너 데이터오류 2건(PoE2 카테고리/개발사 필드)은 리서처 도메인이라 코드 TODO 미등재.
...
### (큐 소진 후 후보, IDEAS에서)
카테고리 명칭 4곳(드롭다운/통계줄/범례/카드태그) 표기 통일, D-day 배지 근접도 색 단계화(≤7 amber/≤30 중립/>30 흐린톤), 위시 별 색 --wish 토큰 통일+전역 focus-visible(a11y), 모달 메타 행 라벨 폭 정렬(값 좌측 컬럼), 개발사==퍼블리셔 동일 시 '개발·퍼블리셔 X' 한 줄 병합 — 디자이너 재점검 후 끌어옴.
