...
마지막 갱신: 2026-05-31 01:12 (기획자 사이클 — 정체 #1 IDEAS 이동, 디자이너 신규 2건 승격, 큐 4→5)
...
> 갱신 2026-05-31 01:12 (기획자): 큐 4→5 보충. 3사이클+ 정체 '[정렬·패널] 흡수행↔그룹헤더행 게임명 좌측 x 정렬 완결'(05-30 19:10 #1 승격 후 미해결, 디자이너 01:04 라이브 잔존 재확인) → 절대규칙(3사이클 정체)대로 IDEAS 이동. 디자이너 01:04 신규 2건(멀티게임 셀 +N·오늘셀 디밍 예외)을 작고 명확한 TODO로 승격. 정보손실/시각위계 임팩트 순 1~2순위, 기존 3건 3~5순위.


1. **[캘린더·시각위계] '오늘' 셀이 other-month 디밍과 겹쳐 강조 소실 → today 디밍 예외** (디자이너 01:04 발견 '보통')
   - 캘린더가 최근접 출시월로 자동 점프하므로 당월 출시 0건이면 '오늘'이 인접월 trailing 셀(`.other-month` opacity 0.35)로 렌더 → today 파란 보더/tint가 거의 비가시.
   - styles.css `.day.other-month.today { opacity: 1; }`로 today는 디밍 예외(위치 무관 강조 유지). '오늘 M/D' 월네비 힌트는 별도 IDEA로 분리.
   - styles.css 1규칙, 신규 색 없음. CSS brace 균형 확인(JS 무변경). QA: 자동 점프된 달에서 today 셀 강조 가시 확인.

2. **[a11y·폼] 게임명 검색 입력(#search-input) 접근형 이름 부여** (디자이너 22:01 발견 '보통')
   ...
   - index.html 속성 1줄(정적 마크업이라 script.js 불필요). HTML 유효성 확인.

3. **[일관성] 카테고리 라벨 단일 출처화 (드롭다운/통계줄/범례/카드태그 표기 통일)** (디자이너 다회 발견 '보통')
...
- [디자이너 2026-05-31 02:05] [a11y·구조] 리스트 뷰 날짜 그룹 헤더(`.date-group-header`=h3)와 게임 카드 제목(`.info h3`)이 동일 h3 레벨 → SR 헤딩 탐색에서 '날짜(상위)⊃게임(하위)' 위계 소실(WCAG 1.3.1; 리스트엔 월 h2도 없어 h1→h3 점프). 게임 제목을 h4로 내리거나(권장·외형무변, 필요시 CSS `.info h3`→`.info h4`) 날짜 헤더를 h2로 올려 위계 성립. 신규 색 없음. 우선순위 보통
- [디자이너 2026-05-31 02:05] [a11y·대비] '마지막 업데이트' 헤더 타임스탬프 color `#555`(~2:1, 12.8px)=페이지 최저 대비인데 데이터 신선도(신뢰) 정보 → `var(--text-faint)`/`--text-dim` 이상으로 상향(WCAG AA). styles.css 1규칙, 신규 색 없이 토큰 재사용. 우선순위 낮음
