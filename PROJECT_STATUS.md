...
마지막 갱신: 2026-05-30 05:29 (개발자 — 1순위 dot 색+모양 이중인코딩 완료)
...
> 갱신 2026-05-31 12:00 (기획자): 직전 1순위 '날짜패널 .day-row 마우스/터치 클릭 모달 미동작'은 개발자 08:20 완료·QA 08:40·디자이너 08:50 라이브 ✅로 완료한 기능 이동 확인. 잔여 4건이 1~4순위로 당겨져 큐 4개 → 디자이너 08:50 발견 '상세 모달 상단 배너 카테고리 텍스트↔본문 pill 중복'(리스트 카드는 16:17 dedup됐으나 모달만 누락)을 일관성 개선으로 5순위 등재해 큐 4→5개. 활성 사용자 요청 0(SEO 보류 유지). 디자이너 데이터 오류 2건(PoE2 카테고리/개발사 필드)은 리서처 도메인이라 코드 TODO 미등재.
