...
### 1순위 — [캘린더] 날짜 클릭 시 '그날 이후 출시' 목록으로 교체 (사용자 요청)
지금은 셀 클릭 시 그날 하루치만 패널에 표시. → 그날(포함) 이후 출시 게임 전체를 날짜별로 묶어 보여주도록 변경.
- `day-detail-panel`에 그날 하루 대신 `release_date >= 클릭한 날짜`인 게임을 날짜순 정렬·날짜별 그룹 헤더로 표시(기존 리스트 뷰의 그룹핑 UI/로직 재활용).
- 패널 제목: `2026.06.04 (목) 이후 출시 N건`. 패널은 세로 스크롤 허용(max-height). 빈 셀도 클릭하면 그 이후 목록이 나오므로 "출시 없음" 허전함 해소.
- 게임 클릭 → 기존 `openModal()` 재사용. 선택 셀 강조(.selected)·× 닫기 유지. 현재 적용된 기간/카테고리/플랫폼 필터는 그대로 반영.
- 회귀 주의: 리스트 뷰 자체는 변경 X. 50줄 넘으면 '데이터 필터'와 '패널 렌더' 두 TODO로 분리.
- 예상 변경: script.js ~30줄, styles.css ~6줄.

### 2순위 — [가독성] 본문/메타 텍스트 대비·크기 상향 (CSS only)
...
### 3순위 — [컨트롤 정리] 검색바 + 필터 한 줄 묶기 (CSS 위주)
...
### 4순위 — [컨트롤 정리] 퀵칩을 필터 행 끝으로 이동 + 모바일 가로 스크롤
...
### 5순위 — [캘린더] 셀 키우고 대표 게임명 1건 텍스트 노출
...
### 6순위 — [정리] 핵심 색을 :root CSS 변수로 1차 토큰화
