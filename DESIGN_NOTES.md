...
_(오래된 1개는 archive/DESIGN_NOTES_2026-06-11.md로 이동)_
...
### E. 헤더 리디자인 (→ §F로 갱신: 미니멀 확정) [2026-06-09 17:58 / 18:10 갱신]
**갱신: §F 3컬럼 도입(카테고리=좌측 레일)으로 헤더 내비가 중복 → 헤더는 미니멀형(워드마크 + 위시·테마토글, 내비 제거)으로 확정.** 아래 내비 포함형 스펙은 보류(랜딩 5라우트는 좌측 카테고리 레일/푸터로 접근).
...
### F. 데스크톱 3컬럼 레이아웃 (운영자 확정 [2026-06-09 18:10]) — §B-3·§E 갱신
운영자: "웹은 3컬럼 — 1컬럼 카테고리 / 2컬럼 본문 / 3컬럼 출시 임박." 목업 확정(헤더 미니멀 동반 확정).
구조(데스크톱 ≥~900px · `components/Home.tsx` 본문 래퍼 + `Home.module.css`): `display:grid; grid-template-columns:~150px 1fr ~190px; gap:~1rem; align-items:start`.
- **좌(카테고리)** — 신규 `components/CategoryRail`(또는 Filters 재구성): 전체/모바일/PC·콘솔/글로벌/신규서버 행(정보색 점 + 라벨 + 카운트), 활성 카테고리 accent 옅은 bg+border(`rgba(47,111,224,.08)`/border `var(--accent)`). 하단에 기간(오늘 이후/전체) 셀렉트. **= 기존 가로 필터바(§B-3) 대체** — 검색/카테고리/기간 한 줄 안 폐기, 카테고리·기간은 좌측 레일로 이관.
- **중앙(본문)** — 상단 1행: 검색 input + 캘린더/리스트 토글. 그 아래 월 이동 + 캘린더(셀=날짜+정보색 점, 게임명은 셀 클릭 시 패널). 기존 단일 컬럼 본문을 중앙 컬럼으로.
- **우(출시 임박)** — 기존 HeroStrip(상단 큰 배너)을 **우측 세로 레일**로 이관: '출시 임박' warm 좌띠 타이틀 + D-day 순 컴팩트 행(점+이름+D-day). 첫 화면에서 본문(캘린더) 즉시 노출·임박은 사이드 상존. 글로우/펄스 제거(B-2) 그대로.
모바일(≤~900px): 3컬럼 → **1컬럼 스택** — 순서 본문(캘린더) → 출시 임박(접이식/축약) → 카테고리(상단 칩/드롭다운 필터). 좌/우 레일은 데스크 전용(`@media`로 그리드 해제).
신규 색 0(정보색은 §A 카테고리 톤다운·라이트 토큰은 §D). 본 레이아웃은 색/플랫화(Phase 1·2) 이후 또는 병행 — 구조 변경이라 별도 큰 단위.

### G. 라이브 중간 점검 — 운영자 "어색해" 원인 (데스크 1440 실측 [2026-06-09 18:35])
개발이 좌측 카테고리 레일+라이트 테마 선반영 → **3컬럼이 좌측만 적용된 중간 상태**라 어색. 원인 3:
1. **[높음·구조] 우측 '출시 임박' 레일 미구현** — HeroStrip이 상단 큰 카드 2개로 잔존(첫 화면 점유) + 좌측 카테고리 레일이 '기간'에서 끝나 **그 아래 페이지 끝까지 빈 흰 세로 거터**(가장 큰 어색함의 주범). 조치: §F대로 출시 임박을 **우측 세로 레일**로 이관(상단 카드 제거) → 우측 채워져 좌·중·우 균형. 좌측 레일은 `position:sticky; top` 처리(짧은 레일이 긴 본문 옆에서 거터로 보이지 않게). `components/Home.tsx`+`Home.module.css` 3컬럼 그리드.
2. **[높음·중복] 헤더 카테고리 내비 ↔ 좌측 레일 카테고리 중복** — 헤더 내비(모바일·PC·콘솔·글로벌)와 좌측 레일이 동일 표시. §E 갱신(헤더 미니멀·내비 제거) 미반영분. 조치: 헤더 내비 제거→워드마크+위시·테마토글만(`app/layout.tsx` `.site-nav` 제거, SEO 랜딩은 푸터/좌레일 링크로).
3. **[보통·색] 좌측 레일 카테고리 점이 아직 원색(쨍함)** — §A 정보색 톤다운 미적용. 조치: 카테고리 토큰을 §A/§D 정보색(라이트: 모바일 #3f7d54·PC #3a6ea5·글로벌 #7e4f99·신서버 #b5601f)으로.
종합: 어색함 = '3컬럼 좌측만 완성+우측 공백 → 좌측 빈 거터' + '헤더 카테고리 중복'. 둘 다 기존 스펙(§F·§E) 미완성분 — 신규 결정 불필요, 개발 이어서 완료하면 해소.

### H. 컬럼 패널 색 구분 + 하단 광고 제거 + 모바일 (운영자 [2026-06-10 09:30])
운영자: "모바일도 수정 / 하단 광고 자리 빼고 / 좌·중·우 컬럼 div로 묶어 색 구분." + "더 좋은 디자인 있으면."
1. **[높음] 3컬럼 패널화** — 각 컬럼을 패널 div로: 페이지 bg 옅은 회색(라이트 `--bg`를 #f6f7f9→#eceef1 검토), 좌(카테고리)/중(본문)/우(출시임박) 각각 `border:1px solid var(--border); border-radius:10px; padding`. `Home.module.css` 컬럼 래퍼. **색 구분 = B 확정(운영자 2026-06-10 09:35 — "B로 가자").** 세 컬럼 **동일 near-white 패널(#fff) + 컬럼 헤더 라벨(카테고리/캘린더/출시임박) + 1px 보더·간격**으로 구획, 페이지 bg 옅은 회색(#eceef1)으로 패널 분리. **색은 의미 있는 곳에만**(카테고리 점·D-day warm·강조 블루). **warm은 출시임박 컬럼 *헤더 띠*에만**(좌측 3px warm 바 + 라벨색 #c47a00 + 헤더 하단 보더 #efe6d4, 패널 바디는 흰색). 각 패널 `.phead`(라벨+하단보더)+`.pbody`. (반려 A: 3색 틴트 — 막 정리한 색 재산만화 위험.) 다크모드는 토큰 전환으로 자동(헤더 bg=`--bg-elev`).
2. **[높음] 하단 광고 제거** — `SeoLanding`/`Home` 하단 `AdSlot`(landing-bottom/mid) 제거(운영자 지시). 상단 슬롯은 유지/별도 판단.
3. **[높음] 모바일 1컬럼 스택** — ≤~900px: 헤더(미니멀) → 카테고리 **가로 스크롤 칩**(전체/모바일/PC·콘솔/글로벌/신서버, 점+라벨) → 검색+토글 → 캘린더 패널 → 출시임박 패널(warm). 좌/우 레일 그리드 해제, 하단 광고 없음. 디자이너 권고: 모바일 기본 뷰를 **리스트**로(좁은 폭 캘린더 셀 과밀) — 캘린더는 토글 유지.
구조(§F·§G)와 같은 트랙(Phase L). 신규 색 0(warm/정보색은 §A·§D 토큰).
...
- **Phase L (구조·큰 단위)**: §F 데스크톱 3컬럼(카테고리 좌레일·본문 중앙·출시임박 우레일) + §E 헤더 미니멀. 구조 변경이라 색/플랫화와 별도 트랙.
- **Phase 3 (밀도·칩·필터)**: B-3은 §F 좌레일로 대체·§A.3 칩 단일언어·B-4 통계 중립·B-5 범례 정리·C-2~5 상세 위계/스펙2열, 섹션 여백 확대.
