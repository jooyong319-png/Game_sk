...
1. **[높음] 3컬럼 패널화** — 각 컬럼을 패널 div로: 페이지 bg 옅은 회색(라이트 `--bg`를 #f6f7f9→#eceef1 검토), 좌(카테고리)/중(본문)/우(출시임박) 각각 `border:1px solid var(--border); border-radius:10px; padding`. `Home.module.css` 컬럼 래퍼. **색 구분 두 안(운영자 택1)**: (A·운영자 요청) 중앙 흰색·좌 중립 틴트·우 warm 틴트(#fbf7ef·border #efe6d4). (B·디자이너 권고) 세 컬럼 **동일 near-white + 컬럼 헤더 라벨(카테고리/캘린더/출시임박) + 보더·간격**으로 구분, warm은 출시임박 헤더 띠에만 → 더 미니멀(Linear/Vercel 톤)·다크모드 전환 깔끔·정리 방향 정합. 권고 B(3색 틴트는 막 정리한 색 재산만화 위험).
